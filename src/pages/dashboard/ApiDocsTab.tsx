...
// The MCP/API documentation tab. `onNotify` surfaces the copy-confirmation toast
// owned by the Dashboard shell.
export default function ApiDocsTab({ onNotify }: { onNotify: (msg: string) => void }) {
  ...
  const curlSnippet = `curl -X POST ${window.location.origin}/api/mcp/scan -H "Content-Type: application/json" -d '{\n  "url": "https://example.com", \n  "apiKey": "YOUR_API_KEY"\n}'`;
...
{`{
  ...
  "authHeader": "Bearer ey..."      // (Optional) Authorization header to pass to the target
                ...
                <button
                  ...
                  onClick={() => copy(schemaSnippet, 'Schema snippet copied to clipboard')}
                >
...
{`{
  ...
  "creditsRemaining": 90
                ...
                <button
                  className="absolute top-2 right-2 bg-[#27272a]/80 hover:bg-[#3f3f46] text-[#a1a1aa] hover:text-white p-1.5 rounded transition-all opacity-0 group-hover:opacity-100 cursor-pointer"
                  onClick={() => copy(responseSnippet, 'Response envelope snippet copied to clipboard')}
                >
                  <Copy className="w-3.5 h-3.5" />
                </button>
              </div>
