...
/** False-positive exclusion ledger: lists active suppression rules and revokes them. */
export default function ExclusionsTab({ userId, suppressRules, onRefresh }: ExclusionsTabProps) {
            ...
            <div key={rule.id} className="bg-black/80 border border-[#27272a]/95 rounded p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-zinc-750 transition-colors">
              ...
              <button
                onClick={async () => {
                  ...
                  try {
                    const delRes = await fetch(`/api/suppressions/${rule.id}?userId=${userId || 'user_default'}`, {
                      method: 'DELETE'
                    });
                    if (delRes.ok) {
                      onRefresh();
