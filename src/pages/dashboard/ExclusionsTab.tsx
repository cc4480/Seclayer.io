...
import { DashboardData } from './useDashboardData.js';

export default function ExclusionsTab({ data }: { data: DashboardData }) {
  const { suppressRules, isDeletingRule, handleRevokeSuppression } = data;
        ...
        <div className="space-y-3">
          {suppressRules.map((rule) => (
            <div key={rule.id} className="bg-black/80 border border-[#27272a]/95 rounded p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-zinc-750 transition-colors">
              ...
              <button
                onClick={() => handleRevokeSuppression(rule.id)}
