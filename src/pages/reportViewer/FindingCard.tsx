...
import SuppressionFooter from './SuppressionFooter.js';
...
function severityBadgeClass(finding: Finding): string {
  if (finding.isFalsePositive) return 'bg-zinc-800 text-zinc-400 border border-zinc-700/60 font-medium';
  if (finding.severity === 'critical') return 'bg-red-500/10 border border-red-500/25 text-red-400 font-bold';
  if (finding.severity === 'high') return 'bg-red-500/10 border border-red-500/20 text-rose-400';
  if (finding.severity === 'medium') return 'bg-amber-500/10 border border-amber-500/20 text-amber-400';
  if (finding.severity === 'low') return 'bg-[#22c55e]/10 text-[#22c55e] border border-[#22c55e]/25';
  return 'bg-black text-[#52525b] border border-[#27272a]';
}

export default function FindingCard({ finding, copiedCodeId, onCopyCode, expanded, onToggleExpand, sup }: FindingCardProps) {
  const severityColor = severityBadgeClass(finding);
  ...
  return (
    ...
    >
      ...
      <SuppressionFooter finding={finding} sup={sup} />
