...
// MCP-server pitch with a tabbed terminal snippet (Claude Code / Cursor / usage).
export default function McpSection({ onNavigate }: { onNavigate: (view: string, arg?: string) => void }) {
  ...
  const tabClass = (tab: string) =>
    `px-3 py-1 font-mono text-[11px] border-b-2 transition-all pb-1.5 ${
      activeMcpTab === tab
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-6 bg-black border border-[#27272a] rounded overflow-hidden p-6 shadow-2xl">
          ...
          <div className="flex space-x-2 border-b border-[#27272a] mb-4 pb-1">
            <button onClick={() => setActiveMcpTab('claude')} className={tabClass('claude')}>
              Claude Code
            </button>
            <button onClick={() => setActiveMcpTab('cursor')} className={tabClass('cursor')}>
              Cursor Config
            </button>
            <button onClick={() => setActiveMcpTab('manual')} className={tabClass('manual')}>
              Usage Syntax
            </button>
          </div>
