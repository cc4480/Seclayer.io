...
export default function ExclusionsTab({ userId, scanCount }: ExclusionsTabProps) {
  const [suppressRules, setSuppressRules] = useState<SuppressionRule[]>([]);
  ...
  const handleDeleteRule = async (ruleId: string) => {
    ...
    try {
      const res = await apiFetch(`/api/suppressions/${ruleId}`, { method: 'DELETE' });
      if (res.ok) fetchSuppressRules();
    } catch (err) {
            ...
            <div key={rule.id} className="bg-black/80 border border-[#27272a]/95 rounded p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-zinc-750 transition-colors">
              <div className="space-y-1.5">
                ...
                <span className="text-[9px] text-[#52525b] font-mono block">
                  Established: {new Date(rule.createdAt).toLocaleString()} &bull; Exclusion ID: {rule.id}
                </span>
              </div>
